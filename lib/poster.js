...
const Poster = {
    ...
    create(openid) {
        ...
        return API.getUserInfo(openid)
            ...
            // 保存用户微信头像
            .then((info) => {
                return this.saveHeadimg(info)
                    .then(createConfig => createConfig)
                    .catch(err => Promise.reject(err))
            })

            // 创建并保存用户海报图片
            .then(config => this.createPoster(config, (err) => {
                console.log('finish', new Date().getTime() - t1);
                // 返回海报图片文件路径
                return !err ? Promise.resolve(config.savePath) : Promise.reject(err);
            }))
